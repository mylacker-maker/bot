...
# Ключ и настройки в точности как в рабочем скрипте
...
# === AI OPENROUTER ФУНКЦИЯ (ИСПРАВЛЕНА ПОД РАБОЧИЙ СКРИПТ) ===
async def ask_ai_openrouter(user_id, user_name, text):
    ...
    try:
        async with aiohttp.ClientSession() as session:
            async with session.post(OPENROUTER_URL, headers=headers, json=data, timeout=60) as response:
...
async def process_message(message):
    ...
    if s.get("ai_mode") == "on":
        ...
        try:
            ...
            bot.send_chat_action(chat_id, "typing")  # ИСПРАВЛЕНО: убран await
...
def main():
    ...
    # ИСПОЛЬЗУЕМ СТАНДАРТНЫЙ И СТАБИЛЬНЫЙ МЕТОД ОПРОСА
    try:
