...
try:
    ...
    print(f"⚠️ sentence-transformers не загружена: {e}")
    EMBED_MODEL = None
...
try:
    ...
    print(f"️ scikit-learn не доступен: {e}")
...
try:
    ...
    print(f"⚠️ NetworkX не доступен: {e}")
...
DEFAULT_SETTINGS = {"length": "medium", "style": "normal", "reactions": "on", "channel": "all"}
...
def rebuild_faiss_index():
    """Перестроить FAISS индекс из фраз"""
    global faiss_index, faiss_phrases
    
    if not FAISS_AVAILABLE or EMBED_MODEL is None:
        return
    ...
    try:
        ...
        print(f"✅ FAISS индекс построен: {len(faiss_phrases)} фраз")
    except Exception as e:
...
def semantic_search(query_text, top_k=10):
    """Семантический поиск похожих фраз через FAISS"""
    if faiss_index is None or EMBED_MODEL is None or not query_text:
        return []
    ...
    try:
        ...
        for i, idx in enumerate(indices[0]):
            if idx < len(faiss_phrases):
                phrase_idx = faiss_phrases[idx]
                score = float(distances[0][i])
                results.append((phrase_idx, score))
...
def build_knowledge_graph():
    ...
    for phrase in model["phrases"][-1000:]:  # Последние 1000 фраз
        text = phrase.get("text", "")
        tokens, lemmas = get_tokens_and_lemmas(text)
        
        # Добавляем узлы для значимых слов
        ...
        for i, word1 in enumerate(meaningful):
            ...
            for word2 in meaningful[i+1:i+3]:  # Связываем с соседними словами
    ...
    print(f"✅ Граф знаний построен: {knowledge_graph.number_of_nodes()} узлов, {knowledge_graph.number_of_edges()} рёбер")

def get_related_words(word, depth=2):
    """Получить связанные слова из графа знаний"""
    if knowledge_graph is None or word not in knowledge_graph:
        return []
    
    related = []
    try:
        neighbors = list(nx.neighbors(knowledge_graph, word))
        for neighbor in neighbors[:5]:  # Топ-5 соседей
            related.append(neighbor)
            # Добавляем соседей соседей
            if depth > 1:
                for second_neighbor in list(nx.neighbors(knowledge_graph, neighbor))[:3]:
                    if second_neighbor != word:
                        related.append(second_neighbor)
    except Exception:
        pass
...
def build_tfidf_clusters():
    """Построить TF-IDF матрицу и кластеризовать фразы"""
    ...
    try:
        ...
        if n_clusters > 1:
            ...
            # Сохраняем кластеры в модель
            model["clusters"] = {}
            for i, cluster_id in enumerate(clusters):
                model["clusters"].setdefault(str(cluster_id), []).append(i)
            
            print(f"✅ TF-IDF кластеризация: {n_clusters} кластеров")
    except Exception as e:
        print(f"TF-IDF clustering error: {e}")

# === УПРАВЛЕНИЕ МОДЕЛЬЮ ===
def extract_topic_tags(lemmas):
    meaningful = [l for l in lemmas if l not in STOPWORDS and len(l) > 3 and l.isalpha()]
    return meaningful[:2]
...
def add_pair(context_text, response_text):
    ...
    for p in model["pairs"]:
        if fuzz.ratio(ctx_lower, p["context"].lower().strip()) > 90:
            return False
    ...
    if len(model["pairs"]) > MAX_PAIRS:
        model["pairs"] = model["pairs"][-MAX_PAIRS:]
    return True
...
def save_json(path, obj):
    ...
    try:
        ...
        print("Ошибка сохранения файла", path, ":", e)
...
def retrieve_phrases(query_lemmas, query_text, limit=20):
    ...
    if query_text and FAISS_AVAILABLE and EMBED_MODEL is not None:
        ...
        for idx, score in semantic_results:
            scores[idx] = scores.get(idx, 0.0) + score * 10.0  # Высокий вес для семантики
...
def get_recent_answers(chat_id):
    return model.get("recent_answers", {}).get(str(chat_id), [])
...
def set_reaction(chat_id, message_id, reaction_emoji):
    """Исправленная функция установки реакции"""
    try:
        # Правильный формат для Telegram Bot API
        reaction = [{"type": "emoji", "emoji": reaction_emoji}]
        
        # Используем requests напрямую для надежности
        import requests
...
def handle_message_reaction(message_reaction):
    try:
        ...
        with model_lock:
            ...
            save_if_needed(True)
    except Exception as e: print("Reaction update error:", e)
    ...
    try:
        ...
        if action == "cat":
            try:
                ...
                if "message is not modified" not in str(e): print("edit cat error:", e)
            try: bot.answer_callback_query(call.id)
            except Exception: pass
            return
        ...
        elif action == "react": current_text, current_markup = behavior_text(chat_id), settings_behavior_markup(chat_id)
        ...
        if current_text and current_markup:
            try: bot.edit_message_text(current_text, chat_id, call.message.message_id, reply_markup=current_markup)
                ...
                if "message is not modified" not in str(e):
                    try: bot.send_message(chat_id, current_text, reply_markup=current_markup)
                    except Exception: pass
...
def process_channel_post(message):
    ...
    except Exception as e: print("Channel post error:", e)
...
if hasattr(bot, "channel_post_handler"):
    ...
    except Exception as e: print("channel_post_handler registration error:", e)
...
def start_polling():
    ...
    offset = None
    # ИСПРАВЛЕНИЕ: убран "sticker" из allowed_updates (стикеры приходят как часть message)
    ...
    while True:
        ...
        if updates:
            for upd in updates:
                ...
                except Exception as e: print("manual message_reaction error:", e)
            try: bot.process_new_updates(updates)
            except Exception as e: print("process_new_updates error:", e)
...
def main():
    ...
    for attempt in range(5):
        try:
            ...
            print("getMe error:", e)
            time.sleep(3)
    if bot_id is None: raise SystemExit("Не удалось получить bot_id через getMe. Проверь токен и интернет.")
    print(f"{BOT_TITLE} запущен (Семантический поиск + Граф знаний + TF-IDF).")
    ...
    except KeyboardInterrupt: print("Останавливаю...")
    except Exception as e: print("Polling error:", e)
    finally: save_all()
